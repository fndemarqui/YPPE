...
#'
#' @export
#' @param x an object of the class summary.yppe.
#' @param ... further arguments passed to or from other methods.
#' @return a summary of the fitted model.
print.summary.yppe <- function(x, ...){
  if(x$approach=="mle"){
    cat("Call:\n")
    print(x$call)
    cat("\n")
...
#'
#' @aliases summary.yppe
#' @export
#' @param object an objecto of the class 'yppe'.
#' @param ... further arguments passed to or from other methods.
#'
summary.yppe <- function(object, ...){
  p <- object$p
  q <- object$q
  if(object$approach=="mle"){
    n <- object$n
    k <- p+2*q+object$n_int
...
#'
#' @export
#' @param object an objecto of the class 'yppeBoot'.
#' @param ... further arguments passed to or from other methods.
#'
summary.yppeBoot <- function(object, ...){
  q <- object$q
  p <- object$p
  n <- object$n
  k <- ncol(object$par)

  loglik <- -mean(object$loglik)
  AIC <- -2*loglik + 2*k
  BIC <- -2*loglik + k*log(n)

  labels <- object$labels

  coefficients <- apply(object$par[ ,1:(2*q+p)], 2, mean, na.rm=TRUE)
  se <- apply(object$par[ ,1:(2*q+p)], 2, stats::sd, na.rm=TRUE)
  ...
  if(p==0){
    ...
    labels.ph <- object$labels.ph
    rownames(TAB3) <- labels.ph
    res <- list(call=object$call,
                coefficients1=TAB1, coefficients2=TAB2, coefficients3=TAB3,
                loglik=loglik, AIC=AIC, approach=object$approach)
  }
