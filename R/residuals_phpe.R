...
residuals.phpe <- function(object, type = c("cox-snell", "martingale", "deviance")){
  type <- match.arg(type)
  mf <- object$mf
  labels <- names(mf)[-1]
  ...
  if(object$approach=="mle"){
    par <- object$fit$par
    for(i in 1:n){
      St[i] <- phpeSurv(time[i], Z[i,], par, rho, tau, n_int)
    }
  }else{ # Bayesian approach
    samp <- rstan::extract(object$fit)
    for(i in 1:n){
      aux <- apply(par, 1, phpeSurv, time=time[i], z=Z[i,], rho=rho, tau=tau, n_int=n_int)
